...
st.write("안내사항: 플로리 행운업 업데이트")
st.write("Update: 2024-12-07")
...
option = st.selectbox(
    ...
    ["가상 크로스", "크로스 조합 찾기 (단일)", "크로스 조합 찾기 (다중)", "우파루 리스트",
     "소환 조건 메모", "필요 먹이량 메모", "농장 생산량 메모", "옵션 물약 기댓값 계산"]
)
...
###############################################################################################
    ...
    st.text(" ")
    st.text("2. 왼쪽 우파루의 유전자(?)를 더 잘 물려받는 경향이 있습니다.")
    st.text("2-1. 대체로 왼쪽 우파루를 크로스 시간이 짧은 우파루(단일속성)으로 배치하면 크로스 시간(기댓값)이 짧아집니다.")    
    st.text(" ")
    st.text("3. 별속성, 3속성, 한정, 레어, 드빌 우파루들은 일반적으로 조합에 관계없이 고정 확률을 지닙니다.")
    st.text("3-1. 한정 3속성은 대체로 3%, 한정 2속성은 5%, 레어 우파루는 4%의 고정확률을 지닙니다.")
    st.text("3-2. 특수 우파루 또한 고정 확률을 지닙니다. (토드 3%, 러브럽 5%, 포기사 5%)")
    st.text("3-3. 단, 조합 결과 상 나올 수 있는 고정 확률 우파루가 너무 많아지면, 전체적으로 확률을 낮춤으로써 총합이 100%가 되도록 유지합니다.")
    st.text(" ")
    st.text("4. 따라서 6속성을 섞는 크로스, 레어 속성 조합을 3가지 이상 포함하는 조합은 대체로 확률 너프를 받습니다.")
    st.text("4-1. 레어 속성 조합: 숲+얼음, 불+얼음, 불+매직, 땅+바람, 물+천둥, 물+슈거")
    st.text("4-2. 행운업을 할 때에는 고정확률의 합 또한 높아지므로, 확률 너프를 받을 가능성이 커집니다.")
    st.text(" ")
    st.text("5. 확률업 이벤트 진행 시에는 매직크로스에만 즉시완료+행운업을 시행하는 것이 훨씬 이득입니다.")
    st.text("5-1. 매직크로스 42시간, 우파루크로스 10시간이더라도, 우파루크로스를 포기하고 매직크로스만 즉시완료로 굴리는 게 확률적으로 이득입니다.")
    st.text("5-2. 하지만 한정을 여러 마리 뽑을 시, 심리적으로는 우파루크로스에도 한정 1마리를 띄우는 게 마음이 편안합니다.")
    st.text(" ")
    st.text(" ")
    st.text(" ")
    ...
    st.text("고대신룡(0.6%, 35시간 57분): 16레벨 이상끼리 크로스, 5속성 이상")
    st.text("다크닉스(0.6%, 35시간 57분): 16레벨 이상끼리 크로스, 5속성 이상")
    st.text("골디(1.2%, 48시간):     레어 1마리 포함, 4속성 이상")
    st.text("다크골디(0.7%, 48시간): 레어+레어 (2속성 크로스인 바우+보라도루 추천)")
    st.text("루핀(1.5%, 40시간):     레어 1마리 포함, 4속성 이상")
    st.text("크로노(2%, 42시간):     5속성 이상")
    st.text("클라우(1.2%, 50시간):   11레벨 이상끼리 크로스, 5속성 이상")
    st.text("치우(1.2%, 50시간):     11레벨 이상끼리 크로스, 바람, 얼음 포함 4속성 이상")
    st.text("홀리(1.2%, 50시간):     11레벨 이상끼리 크로스, 물, 천둥 포함 4속성 이상")
    st.text("레오(1%, 51시간):       16레벨 이상끼리 크로스, 레어 1마리 포함, 4속성 이상")
    st.text("아르코(1%, 52시간):     16레벨 이상끼리 크로스, 물 포함 5속성 이상")
    st.text("루루(1.5%, 40시간 15분): 불, 바람, 매직 포함 4속성 이상")
    st.text(" ")
    st.text("신성 5종을 모두 노리려면?")
    st.text("16레벨 이상끼리 크로스, 물, 천둥, 바람, 얼음 포함 5속성 크로스")
    st.text("(6속성 크로스할 경우에는 확률 너프 감수)")
    st.text(" ")
    st.text(" ")
    st.text(" ")
    ...
    st.text(" ")
    st.text("아트모스: 매그너스, 라푼타, 마르스, 미라쥬, 엔젤 보유 시 소환 가능")
    ...
    st.text("(네로는 진화 성공체 중에서도 실패작이라 생각하시면 편합니다.)")
    st.text(" ")
    st.text("숲, 땅, 불, 얼음, 번개, 물, 바람, 매직 마법석을 주면 수당: 진화확률 +3%")
    st.text("별 마법석 1개를 주면: 진화확률 +15%")
    st.text("검은 마법석 1개를 주면: 진화확률 +15%")
    st.text("불 마법석 1개를 주면, 진화 선공 시 등장 확률이 실비아 +3%, 네로 -3%")
    st.text("바람 마법석 1개를 주면, 진화 선공 시 확률이 루나실 +3%, 네로 -3%")
    st.text("매직 마법석 1개를 주면, 진화 선공 시 확률이 미쉘 +3%, 네로 -3%")
    st.text("검은 마법석 1개를 주면, 진화 선공 시 확률이 실비아 +5%, 루나실 +5%, 미쉘 +5%, 네로 -15%")
    st.text(" ")
    st.text("(마법석은 하루에 3개까지만 투입 가능)")
    st.text("불 마법석 30개 지급: 진화확률 100%")
    st.text("=> 실비아 93%, 루나실 3%, 미쉘 3%, 네로 1%")
    st.text("바람 마법석 30개 지급: 진화확률 100%")
    st.text("=> 실비아 3%, 루나실 93%, 미쉘 3%, 네로 1%")
    st.text("매직 마법석 30개 지급: 진화확률 100%")
    st.text("=> 실비아 3%, 루나실 3%, 미쉘 93%, 네로 1%")
    st.text("검은 마법석 6개 지급: 진화확률 100%")
    st.text("=> 실비아 33%, 루나실 33%, 미쉘 33%, 네로 1%")
    st.text(" ")
    st.text("등장 확률이 음수가 되면 0%로 조정.")
    st.text("등장 확률 합이 100%를 초과하면 비중대로 부여하여 100%로 맞춤")
    st.text("예: 불 마법석 30개 지급 후 검은 마법석 1개를 주면,")
    st.text(".    실비아 93% -> 98%, 루나실 3% -> 8%, 미쉘 3% -> 8%, 네로 1% -> -14%")
    st.text(".    네로는 -14% -> 0%로 조정")
    st.text(".    실비아는 98% -> 98/(98+8+8) = 86%로 조정")
    st.text(".    루나실과 미쉘은 8% -> 8/(98+8+8) = 7%로 조정")
